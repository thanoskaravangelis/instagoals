...
class Goals: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        detector = GestureDetectorCompat(this,GestureListenerGoals())
        ...
        go_to_new.setOnClickListener{
            ...
            val view = layoutInflater.inflate(R.layout.addnewgoal,null)
            ...
            TransitionManager.beginDelayedTransition(goals)
            popupWindow.showAtLocation(goals, Gravity.CENTER,25,25)

        }

        /*val con_layout : ConstraintLayout = findViewById<ConstraintLayout>(R.id.goals)
    ...
    inner class GestureListenerGoals : GestureDetector.SimpleOnGestureListener() {
            ...
            return if (Math.abs(diffX) > Math.abs(diffY)) {
                if (Math.abs(diffX) > SWIPE_THRESHOLD && Math.abs(velX) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffX > 0) { //right swipe
                        ...
                        this@Goals.onSwipeRight()
                    } else { //left swipe
                        Log.i("swipeleft","swipe left detected")
                        this@Goals.onSwipeLeft()
                    }
                    true
                }
                else {
                ...
                if (Math.abs(diffY) > SWIPE_THRESHOLD && Math.abs(velY) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffY > 0) {
                        this@Goals.onSwipeTop()
                    } else {
                        this@Goals.onSwipeBottom()
                    }
                    true
                } else {
                    super.onFling(touchevent1, touchevent2, velX, velY)
